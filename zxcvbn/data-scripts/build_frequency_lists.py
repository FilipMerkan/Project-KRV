...
DICTIONARIES = dict(
    ...
    male_names        = None,
    female_names      = None,
    slovenian_surnames = None,  # Include all Slovenian surnames
    slovenian_male_names = None,  # Include all Slovenian male names
    slovenian_female_names = None,  # Include all Slovenian female names
    slovenian_wikipedia = 30000,  # Slovenian word frequencies (limit to 30k)
    slovenian_passwords = 30000,  # Slovenian common passwords (limit to 30k)
)
...
def filter_frequency_lists(freq_lists):
    ...
    for name, token_to_rank in iteritems(freq_lists):
        for token, rank in iteritems(token_to_rank):
    ...
    for name, token_to_rank in iteritems(freq_lists):
        for token, rank in iteritems(token_to_rank):
            ...
            token_count[name] += 1
    result = {}
    for name, token_rank_pairs in iteritems(filtered_token_and_rank):
...
def main():
    ...
    with codecs.open(output_file, 'w', 'utf8') as f:
        ...
        lines = []
        for name, lst in iteritems(freq_lists):
